...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, input_resolution):
        ...
        shortcut = x
        x = self.norm1(x)
        if H0 % self.window_size != 0 or W0 % self.window_size != 0:
            x = x.view(B, H0, W0, C).permute(0, 3, 1, 2)
            # reflect pad feature maps to multiples of window size
            pad_l = pad_t = 0
            ...
            # print("padding", pad_l, pad_r, pad_t, pad_b)
            x = x.permute(0, 2, 3, 1)
            H = H0 + pad_b
            W = W0 + pad_r
        else:
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, input_resolution, slice_size=None):
        H0, W0 = input_resolution
        device = x.get_device()
        if input_resolution != self.input_resolution:
            self.update_mask(input_resolution, device)
        if slice_size is not None:
            inter_slice_mask = self.get_inter_slice_mask(input_resolution, slice_size, device)
        B, L, C = x.shape
        ...
        if self.shift_size > 0:
            ...
            if slice_size is None:
                ...
                mask = (self.attn_mask + inter_slice_mask).clip(-100.0, 0.0)
        else:
            shifted_x = x
            assert (slice_size is None) or (slice_size[0] % self.window_size == 0)
            mask = None
        ...
        attn_windows = self.attn(x_windows,
                                 mask=mask)  # nW*B, window_size*window_size, C
    ...
    def update_mask(self, input_resolution, device):
    ...
    def get_inter_slice_mask(self, input_resolution, slice_size, device):
        H, W = input_resolution
        Hs, Ws = slice_size
        nH = H // Hs
        nW = W // Ws
        H = H + (self.window_size - H % self.window_size) % self.window_size
        W = W + (self.window_size - W % self.window_size) % self.window_size
        img_mask = torch.arange(nH * nW).reshape(nH, nW).repeat_interleave(Hs, dim=0).repeat_interleave(Ws, dim=1)
        img_mask = img_mask.reshape(1, H, W, 1)
        if self.shift_size > 0:
            img_mask = torch.roll(img_mask, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        img_mask = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, 1
        img_mask = img_mask.view(-1, self.window_size * self.window_size)
        attn_mask = img_mask.unsqueeze(1) - img_mask.unsqueeze(2)
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
        inter_slice_attn_mask = attn_mask.to(device)
        return inter_slice_attn_mask


if __name__ == '__main__':
    layer = SwinTransformerBlock(dim=96, num_heads=3, window_size=4, shift_size=2).cuda()
    x = torch.randn(1, 64, 96).cuda()
    layer(x, (8, 8))
    layer.get_windows_mask((8, 8), (2, 2))
